...
class ComplementBall(RiemannianSubmanifold):
    ...
    def exp(self, point, tangent_vector):
        v = point + tangent_vector
        #norm_v = np.linalg.norm(v,axis=0)
        #print(norm_v)
        return v
    #v*(norm_v>self._min_norm) + v*(self._min_norm/norm_v + self._tol)*(norm_v>=self._min_norm) 
    ...
    class Ball(RiemannianSubmanifold):
        ...
        def projection(self, point, vector):
            norm_p = np.linalg.norm(point,axis=0)
            if True in (norm_p > self._max_norm - self._tol).tolist():
                return (vector - self.inner_product(vector,point)*point/(norm_p + 10**-14))*(norm_p > self._max_norm - self._tol) + vector*(norm_p <= self._max_norm - self._tol)
            else:
                return vector
        ...
        def exp(self, point, tangent_vector):
            norm_p = np.linalg.norm(point,axis=0)
            if True in (norm_p > self._max_norm - self._tol).tolist():
                v = (tangent_vector - self.inner_product(point,tangent_vector,point)*point/(norm_p + 10**-14))*(norm_p > self._max_norm - self._tol) + tangent_vector*(norm_p <= self._max_norm - self._tol)
                return v/(np.linalg.norm(v,axis=0)+10**-14)
            else:
        ...
        def log(self, point_a, point_b):
            norm_p = np.linalg.norm(point_a,axis=0)
            if True in (norm_p > self._max_norm - self._tol).tolist():
                v = (point_b-point_a - self.inner_product(point_a,point_b-point_a,point_a)*point_a/(norm_p+10**-14))*(norm_p > self._max_norm - self._tol) + (point_b-point_a)*(norm_p <= self._max_norm - self._tol)
                return v/(np.linalg.norm(v,axis=0)+10**-14)
            else:
        ...
        def random_point(self):
            v = np.random.normal(size=self._shape)
            norm_p = np.linalg.norm(v,axis=0)
            return v/norm_p*(self._max_norm - self._tol)
