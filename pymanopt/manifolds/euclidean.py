...
class SkewSymmetric(_Euclidean):
    ...
    class EuclideanSubspace(_Euclidean):
        def __init__(self, name, dimension,proj, *shape):
            self._shape = shape
        ...
        def euclidean_to_riemannian_gradient(self, point, euclidean_gradient):
            return self.projection(euclidean_gradient)

        def euclidean_to_riemannian_hessian(
            self, point, euclidean_gradient, euclidean_hessian, tangent_vector
        ):
            return self.projection(euclidean_hessian)
        ...
        def random_point(self):
            return self.projection(np.random.normal(size=self._shape))

        def random_tangent_vector(self, point):
            tangent_vector = self.projection(self.random_point())
            return tangent_vector / self.norm(point, tangent_vector)
